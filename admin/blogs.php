...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .topbar { background: #fff; padding: 16px 24px; border-bottom: 1px solid #e5e7eb; }
        ...
        .blog-item { border: 1px solid #e5e7eb; border-radius: 10px; padding: 14px; }
        .blog-item h3 { margin: 0 0 6px; }
        .blog-meta { color: #6b7280; font-size: 13px; margin-bottom: 8px; }
        .blog-img { max-width: 220px; border-radius: 8px; margin-top: 8px; }
                        ...
                        <div class="blog-list">
                            <?php foreach ($blogs as $blog): ?>
                                <div class="blog-item">
                                    ...
                                    <div class="blog-meta">Slug: <?= htmlspecialchars($blog['slug'] ?? '-', ENT_QUOTES, 'UTF-8') ?> · Created: <?= htmlspecialchars($blog['created_at'] ?? '-', ENT_QUOTES, 'UTF-8') ?></div>
                                    <?php if (!empty($blog['image'])): ?>
                                        <img class="blog-img" src="<?= htmlspecialchars($blog['image'], ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($blog['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                    <?php endif; ?>
                                    <p><?= htmlspecialchars(substr(strip_tags($blog['content'] ?? ''), 0, 180), ENT_QUOTES, 'UTF-8') ?>...</p>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
